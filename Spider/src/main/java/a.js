...
(function() {
    ...
    // an event record
    function KEvent() {
        this.key = 0;                   // ( system | target | source )
        this.count = 0;
        this.remaining = 0;
        this.end_time = 0;
    ...
    // start time should be the start of the hour (utc)
    ...
    Simulator.prototype.fetch = function(now) {
        ...
        function process_event_data(data)
        {
            ...
            _.each(flushed, function(ke) {
                ...
                var system_id = (e >> 16) & 0xff;
                var target_key = (e >> 8) & 0xff;
                var source_key = (e >> 0) & 0xff;

                var should_count_event = true;
                if (system_id == 8 && ke.coords)
                    should_count_event = false;
                if (system_id == 8 && target_key == 0)
                    should_count_event = false;

                if (should_count_event) {
                    var system = GTW.systems[system_id];
                    ++system.count;
                    ++system.target_count[target_key];
                    ++GTW.total_target_count[target_key];
                }
            });
        }

        var url = '/assets/data/events/'+hour+'.json';
